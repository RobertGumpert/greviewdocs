...
> v escapes to heap — переменная улетает в кучу (GC будет за ней следить).
> v does not escape — переменная остается на стеке (очень быстро, очистится сама при выходе из функции).
```
...
то есть произошло роавно тоже самое, что мы уже видели в примерах ранее: 
...
## Slice, Array, Map: кто вы? 



---

## Кто ты err? Кто ты iface и face?
