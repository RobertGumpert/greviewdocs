...
### Но когда появляется указатель?

Теперь давай посмотрим в каких случаях получим указатель, когда объект окажется в куче и когда стоит использовать указатель. Ты удивишься, но всегда указатель = равно размещение в куче :)
...
0000000000498280 <main.main>:
	…
	// main резервирует 0x130 байт
	// rsp+0x00 — это и есть return area для _foo
	subq	$0x130, %rsp
	callq	0x498380 <main._foo>
	…
	leaq 0xa0(%rsp), %rdi  // КУДА: адрес локальной переменной 'user' 
	leaq 0x8(%rsp), %rsi    // ОТКУДА: return area (куда записала _foo)
	…
	// копирует в user используя регистры rdi, rsi
	callq	0x474a42 <runtime.duffcopy+0x302>
	…
	leaq	0x8(%rsp), %rdi	// КУДА: адрес локальной переменной 'user' 
  	leaq	0xa0(%rsp), %rsi  // ОТКУДА: return area (куда записала _foo)
	…
...
0000000000498380 <main._foo>:
 	pushq	%rbp
  	movq	%rsp, %rbp
	// 0x10(%rsp) = rsp_main + 0x08 — это стек main!
  	movups	%xmm15, 0x10(%rsp)
	…
	// _foo не делает subq для своего буфера — своего буфера нет!
	…
...
0000000000498320 <main._foo_1>:
  	cmpq	0x10(%r14), %rsp
  	jbe	0x49836b <main._foo_1+0x4b>
  	pushq	%rbp
  	movq	%rsp, %rbp
  	subq	$0x28, %rsp
  	movups	%xmm15, 0x18(%rsp)
	//
	// 1) 0x00 - 0x28: Собственный кадр _foo_1 (выделен инструкцией subq $0x28).
	// 2) 0x28 - 0x30: Сохраненный rbp (инструкция pushq %rbp занимает 8 байт).
	// 3) 0x30 - 0x38: Адрес возврата в main (положен инструкцией callq).
	// 4) 0x38: Начало области аргументов main.
	// Так как поле ID — первое в структуре User, оно лежит 
	// по самому первому адресу области аргументов.
	// Получается мы читаем из области аргументов в стеке main!
  	movq	0x38(%rsp), %rax
	...
	// формируется eface или же any или interface{}
  	...
  	movq	%rax, 0x20(%rsp) // в регистр помещается указатель на data то есть указатель на uint64 (user.ID *uint64)
	…
